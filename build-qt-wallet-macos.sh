...
xcrun altool --notarize-app -f ../target/mwc-qt-wallet.dmg --primary-bundle-id com.yourcompany.mwc-qt-wallet -u mimblewimblecoin2@protonmail.com -p aikg-yylm-izhr-olai > /tmp/notarize_out.log 2>&1
REQUEST_UUID=`awk '/RequestUUID/{print $NF}' /tmp/notarize_out.log`
echo $REQUEST_UUID;
echo "Sleeping for 1 minute to let apple process things."
sleep 60;
echo "Running: xcrun altool --notarization-info $REQUEST_UUID -u mimblewimblecoin2@protonmail.com -p aikg-yylm-izhr-olai"
xcrun altool --notarization-info $REQUEST_UUID -u mimblewimblecoin2@protonmail.com -p aikg-yylm-izhr-olai
echo "altool notariazation-info complete";
echo "Sleeping for 1 minute to let apple process things."
sleep 60;
